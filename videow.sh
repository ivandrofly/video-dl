...
# v3.3.2 Squashed some other bugs, fixed download of 302 videos on Mac OS X (curl.exe redirection).
...
echo -n "Self-updating script..." && dl http://daniilgentili.magix.net/android/video.sh $ME $Q 2>/dev/null;chmod 755 $ME 2>/dev/null; lineclear
...
$u"
  ...
  wget.exe -S --tries=3 --spider "$u" 2>&1 | grep -q '200 OK\|206 Partial' && base="$base
$u"
  }; done
 }
...
[ "$apple" != "" ] && for a in $apple; do echo "Medium-low quality  (apple streaming, pseudo-m3u8, m3u8) $a";done
 ...
 rai() { 
  ...
  file=$(wget.exe "$saferai" -qQ 50000000 -O -)
 ...
 rai_normal() {
  ...
  echo "$1" | grep -q "rainews.it" && {
   ...
   echo "$file" | grep -q videoURL || { 
    ...
    file="$(wget.exe -Q 50000000 -qO- $urls)"
   }
  }
 ...
 replay() {
  ...
  # Get the json
tmpjson="$(wget.exe http://www.rai.it/dl/portale/html/palinsesti/replaytv/static/"$ch"_$day.html -qO- | tr '\n' ' ')"
...
/g')"
 ...
 # Resolve relinker
  ...
  for f in $(echo $* | awk '{ while(++i<=NF) printf (!a[$i]++) ? $i FS : ""; i=split("",a); print "" }'); do
   ...
   base="$(curl.exe "$dl" | sed 's/ /%20/g')" && checkurl
   # 1st method

   url="$(timeout -skill 5s wget.exe -qO- "$dl&output=25")
 $(timeout -skill 5s wget.exe "$dl&output=43" -U="" -q -O -)"
...
${base//$t\.mp4/$i\.mp4}"; tbase="$(echo "$tbase" | grep -Ev "_([0-9]{3,4})_([0-9]{3,4})\.mp4" | awk '!seen[$0]++')"; done;done
 ...
 lasette() {
  # Store the page in a variable
  page="$(wget.exe "$1" -Q 50000000 -q -O -)"

  # Get the javascript with the URLs
  unformatted="$(wget.exe -q -O - $(echo "$page" | sed '/starter/!d;s/.*src\=\"//;s/\".*//') | sed '/src:.*\|src_.*/!d;s/.*\: \"//;s/\".*//')"
 ...
 mediaset() {
  ...
  unformatted="$(wget.exe "http://cdnselector.xuniplay.fdnames.com/GetCDN.aspx?streamid=$id" -O - -q -U="" | sed 's/</\
/g' | grep http | sed 's/.*http/http/g;s/\".*//g')"
 ...
 dplay() {
  id=$(curl.exe -Ls "$dl" | sed '/data-video-id\=\"/!d;s/.*data-video-id\=\"//g;s/\".*//g')
  [ "$id" = "" ] && {
   orig=$(curl.exe -Ls "$dl" | sed '/dplay.com/!d;s/.*href="http/http/g;s/".*//g') && 
   id=$(curl.exe -Ls "$orig" | sed '/data-video-id\=\"/!d;s/.*data-video-id\=\"//g;s/\".*//g')
  }
  json="$(curl.exe -Ls http://it.dplay.com/api/v2/ajax/videos?video_id=$id)"
...
/g')
    ...
    [ "$url" != "" ] && {
size=
timeout -skill 3s wget.exe -S --spider "$url" &>/dev/null && size=", $(wget.exe -S --spider "$url" 2>&1 | sed '/^Length\|^Lunghezza/!d;s/.*(//;s/).*//')B" || size=", Unkown size"
...
size="$(wget.exe -S --spider "$dl" 2>&1 | sed '/^Length\|^Lunghezza/!d;s/.*[(]//g;s/[)].*//g')"
